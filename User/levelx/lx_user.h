...
// 注意开启了直接映射缓存则需要设置Cache大小为总页数
#define LX_NAND_SECTOR_MAPPING_CACHE_SIZE             (NAND_BLOCK_NBR * NAND_BLOCK_SIZE)
...
// 开启直接映射缓存,否则写未映射的sector会耗时数百ms(因为每次_lx_nand_flash_logical_sector_find会遍历全部block)
