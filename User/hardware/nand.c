...
{
    // MT29F4G08,8位数据宽度,2*2048*64*(2048+64)=512+16MB fmc_ker_ck=200MHz(5ns)
    // 设置NAND参数
    ...
    hNAND.Init.TCLRSetupTime        = 4;                                    // tCLR=10ns(Min), 实测取25ns=5个fmc_ker_ck(设置值4+1)(未开启SDRAM+LTDC的话可取10ns)
    hNAND.Init.TARSetupTime         = 4;                                    // tAR =10ns(Min), 同上
    // 存储组织参数
    hNAND.Config.BlockNbr           = NAND_BLOCK_NBR;   // block总数
    ...
    hNAND.Config.PlaneNbr           = NAND_PLANE_NBR;   // plane数
    ...
    ComSpace_Timing.SetupTime       = 3;    // 建立时间=tALS-tWP(二者都是Min=10ns), 实测取20ns=4个fmc_ker_ck(设置值3+1)(未开启SDRAM+LTDC的话可取5ns)
    ComSpace_Timing.WaitSetupTime   = 4;    // 等待时间=tWP=10ns(Min)(tRP也是10ns), 理论可取2个fmc_ker_ck, 但实测需要至少5个CLK(设置值4+1)
    ComSpace_Timing.HoldSetupTime   = 2;    // 保持时间=tWH=7ns(Min)(tREH也是7ns), 取10ns=2个fmc_ker_ck(设置值2)
    ComSpace_Timing.HiZSetupTime    = 2;    // 高阻时间=tALS-tDS(Min分别为10ns,7ns), 实测取10ns=2个fmc_ker_ck(设置值2)(未开启SDRAM+LTDC的话可取5ns)
...
HRESULT NAND_Read_Page(uint32_t page, uint32_t column, uint8_t* buffer, uint32_t length)
{
    ...
    *(__IO uint8_t *)((uint32_t)(NAND_DEVICE | CMD_AREA)) = NAND_CMD_AREA_A;
    ...
    )
    {
        ...
        for(int i = 0; i < 24; i++)
        {
            ecc_data_spare[i] = *(uint8_t*)NAND_DEVICE;
        }
        // ECC校验
...
HRESULT NAND_Write_Page(uint32_t page, uint32_t column, uint8_t* buffer, uint32_t length)
{
    ...
    *(__IO uint8_t *)((uint32_t)(NAND_DEVICE | CMD_AREA)) = NAND_CMD_WRITE_TRUE1;
    ...
    delay_ns(100);
    // 等待完成tPROG=600us(Max)
    if (nand_wait_ready(800000) != NAND_READY)
    {
        return HR_ERROR;
    }
...
HRESULT NAND_Erase_Block(uint32_t block)
{
    ...
    *(__IO uint8_t *)((uint32_t)(NAND_DEVICE | CMD_AREA)) = NAND_CMD_ERASE1;
    ...
    delay_ns(100);
    // 等待完成tBERS=3ms(Max)
    if (nand_wait_ready(5000000) != NAND_READY)
    {
        return HR_ERROR;
    }
...
{
    for(int i = 0; i < NAND_BLOCK_NBR; i++)
    {
...
static uint32_t nand_wait_ready(uint32_t timeout_ns)
{
    ...
    timeout_ns = ns_to_cpu_tick(timeout_ns);
    while((get_cpu_tick() - start_tick) < timeout_ns)
    {
